...
type Props = { state: BootstrapState; item?: RecordItem; onSaved: (item: RecordItem) => void; onCancel: () => void }
...
export function RecordForm({ state, item, onSaved, onCancel }: Props) {
  ...
  async function submit(event: FormEvent) {
    ...
    set(['Tags', 'tags'], tagsField?.selectOptions.length ? tags.split(',')[0]?.trim() : tags.split(',').map((tag) => tag.trim()).filter(Boolean))
  ...
  return <form className="record-form" onSubmit={submit}>
    ...
    <div className="form-grid"><label>Location<input value={location} onChange={(event) => setLocation(event.target.value)} /></label><label>Tags<input value={tags} onChange={(event) => setTags(event.target.value)} placeholder="patio, cozy" /></label></div>
