...
function App() {
  ...
  return <div className="app-shell">
    ...
    <section className="toolbar"><div className="collection-tabs"><button className={activeCollection === 'all' ? 'tab active' : 'tab'} onClick={() => setActiveCollection('all')}>Everything</button>{bootstrap.collections.map((collection) => <button key={collection} className={activeCollection === collection ? 'tab active' : 'tab'} onClick={() => setActiveCollection(collection)}>{collection}</button>)}</div><input className="search-input" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search names, notes, seasons..." /></section>
    ...
    {editing && <div className="modal-backdrop"><RecordForm state={bootstrap} item={editing.id ? editing : undefined} onSaved={saveRecord} onCancel={() => setEditing(undefined)} /></div>}
  </div>
}
